...
testrealign_scons   = 'testrealign.py'
segemehl_scons = 'segemehl.py'
...
if env['CIRCRNA_METHODS'] == [''] or env['CIRCRNA_METHODS'] == '' or \
   any([f in env['CIRCRNA_METHODS'] for f in ['testrealign', 'circexplorer2_segemehl']]):
    ...
    segemap = env.SConscript(os.path.join(build_dir, segemehl_scons),
    ...
    if env['CIRCRNA_METHODS'] == [''] or env['CIRCRNA_METHODS'] == '' or \
       'testrealign' in env['CIRCRNA_METHODS']  or \
	   'circexplorer2_segemehl' in env['CIRCRNA_METHODS']:

        env_testrealign = env.Clone()
        env_testrealign['BED'] = [segemap['SINGLE_SPLITS'], segemap['TRANS_SPLIT']]
    ...
    if env['CIRCRNA_METHODS'] == [''] or env['CIRCRNA_METHODS'] == '' or \
	   'circexplorer2_segemehl' in env['CIRCRNA_METHODS']:
        ...
        circexplorer2_env['FUSION_FILE'] = testrealign['OLD_BED'] #segemap['SINGLE_SPLITS']
        circexplorer2_env['ALIGNMENTS'] = testrealign['CIRC_READS'] #segemap['ALIGNMENTS']
        ...
        Depends(circexplorer2_segemehl['CIRCRNAS'], testrealign['OLD_BED'])


if env['CIRCRNA_METHODS'] == [''] or env['CIRCRNA_METHODS'] == '' or \
   'findcirc' in env['CIRCRNA_METHODS']:
    ## FIND_CIRC
    findcirc = 'findcirc.py'
    ...
    find_circ = env.SConscript(os.path.join(build_dir, findcirc),
                               variant_dir = build_dir, src_dir = SRC_DIR, 
                               duplicate = 0, 
...
if env['CIRCRNA_METHODS'] == [''] or env['CIRCRNA_METHODS'] == '' or \
   any([f in env['CIRCRNA_METHODS'] for f in ['dcc', 
    ...
    star = env.SConscript(os.path.join(build_dir, 'star.py'),
                          variant_dir = build_dir, src_dir = SRC_DIR,
                          duplicate = 0, 
    		      exports = '''star_env''')	
...
if env['CIRCRNA_METHODS'] == [''] or env['CIRCRNA_METHODS'] == '' or \
    ...
    bwa = env.SConscript(os.path.join(build_dir, 'bwa.py'), 
                          variant_dir = build_dir, src_dir = SRC_DIR, 
                          duplicate = 0, 
                          exports = 'bwa_env')
    ...
    if env['CIRCRNA_METHODS'] == [''] or env['CIRCRNA_METHODS'] == '' or \
       'ciri' in env['CIRCRNA_METHODS']:
        ...
        ciri = env.SConscript(os.path.join(build_dir, 'ciri.py'), 
                              variant_dir = build_dir, src_dir = SRC_DIR, 
                              duplicate = 0, 
                              exports = 'env_ciri')
...
if env['CIRCRNA_METHODS'] == [''] or env['CIRCRNA_METHODS'] == '' or \
    ...
    tophat = env.SConscript(os.path.join(build_dir, 'tophat.py'),
                              variant_dir = build_dir, src_dir = SRC_DIR,
...
circ_met_dict = {'CE2_BWA': circexplorer2_bwa,
                 ...
                 'CIRI'   : ciri,
                 'FC'     : find_circ,
                 'TR'     : testrealign if 'testrealign' in env['CIRCRNA_METHODS'] else None,
                 'DCC'    : dcc,
                 'CFINDER': cfinder
                }
