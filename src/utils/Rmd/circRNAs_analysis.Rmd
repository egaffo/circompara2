...
vars.to.df <- function(vars.py.filepath){
    ...
    a <- gsub(" ", "", gsub("['\"]\\\\[ ]*\n[ ]*['\"]", "", vars))
    ## skip comment lines
    b <- grep(pattern = "^#", invert = T, 
              x = strsplit(a, "\n", fixed = T)[[1]], 
              value = T)
    ## skip blank lines
    c <- paste0(b[b != ""], collapse = "\n")
...
## Reliable circRNAs for each method

```{r}
circ.per.method[, reliable.circ := "Non-reliable"]
circ.per.method[circ_id %in% as.character(circrnas.morethan2methods.medrpm.table$circ_id), 
                reliable.circ := "Reliable"]

ggplot(data = circ.per.method[, .N, by = .(method, reliable.circ)], 
       mapping = aes(x = method, y = N, fill = reliable.circ)) + 
    geom_col(width = .7) + 
    geom_text(aes(label = N), 
              # vjust = -0.3, 
              position = position_stack(vjust = .5),
              size = default.font.size*(5/14)) + 
    geom_text(aes(label = "", y = N*1.1), 
              position = position_stack(vjust = .5),
              size = default.font.size*(5/14)) + 
    scale_y_continuous(expand = c(0, 0)) + 
    scale_fill_viridis_d(name = "CircRNA",
                         option = "D", begin = .35, end = .7, alpha = .8) +
    xlab(NULL) + 
    ylab("CircRNAs") +
    # ggtitle(bquote("CircRNAs detected with " >= .(min_reads) ~ "reads by each method")) + 
    theme_bw() + 
...
```{r, fig.width=2}
plot.circ.lin.xpr <- copy(circ.lin.xpr)
new.inf <- 2*max(plot.circ.lin.xpr[CLR != Inf, CLR])
plot.circ.lin.xpr[CLR == Inf, CLR := new.inf]
```

NB: in plots, infinite CLRs (i.e. when no linear expression was detected for a circRNA) have been represented with double the maximum CLR detected.  

```{r, fig.width=2}
ggplot(plot.circ.lin.xpr, 
       aes(x = NULL, y = log2(CLR))) +
...
ggplot(plot.circ.lin.xpr, 
    ...
    geom_boxplot(varwidth = T, outlier.size = .5) +
    xlab(NULL) +
    ylab(expression(Log[2](CLR))) +
    scale_fill_manual(values = setNames(unique(intgroup.dt[, hue]), 
                                        nm = unique(intgroup.dt[, condition])), 
...
```{r}
ggplot(plot.circ.lin.xpr, 
       aes(x = sample_id, y = log2(CLR), fill = sample_id)) +
    geom_boxplot(varwidth = T, outlier.size = .5) +
    facet_grid(cols = vars(condition), space = "free",
               drop = T, 
               scales = "free_x") +
    xlab(NULL) +
    ylab(expression(Log[2](CLR))) +
