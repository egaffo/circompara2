...
if(basename(meta.file) == "read_groups.info"){
    ...
    gene_meta <- unique(fread(file = meta.file))
}
if(!"condition" %in% colnames(gene_meta)){
  gene_meta$condition <- "cond01"
}
gene_meta <- unique(gene_meta[, .(sample_id = sample, condition)])
