...
option_list <- list(
    ...
    make_option(c("-r", "--min_reads"), action = "store", type = "integer",
                default = 2,
                help = "(DEPRECATED) The minimum detection read threshold for a circRNA  (in at least one sample)"),
    make_option(c("-m", "--min_methods"), action = "store", type = "integer",
                default = 2,
                help = "(DEPRECATED) Keep circRNAs commonly detected by >= m circRNA detection methods (in at least one sample)"),
